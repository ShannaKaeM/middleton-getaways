/**
 * Borders Primitive Editor JavaScript
 * Following the established pattern from primitive-spacing.js and primitive-typography.js
 */
...
jQuery(document).ready(function($) {
    ...
    function bindEvents() {
        ...
        $('.preview-width, .preview-style, .preview-radius, .preview-color').on('change', updateLivePreview);
    ...
    function updateTokenPreview($input, value) {
        ...
        switch(category) {
            case 'widths':
                $tokenItem.find('.border-width-preview').css('border-bottom-width', value);
                break;
                
            case 'styles':
                $tokenItem.find('.border-style-preview').css('border-style', value);
                break;
                
            case 'radii':
                $tokenItem.find('.border-radius-preview').css('border-radius', value);
                break;
        }
    }
    ...
    function updateLivePreview() {
        ...
        const width = $(`.token-input[data-category="widths"][data-slug="${widthSlug}"]`).val() || '2px';
        const style = $(`.token-input[data-category="styles"][data-slug="${styleSlug}"]`).val() || 'solid';
        const radius = $(`.token-input[data-category="radii"][data-slug="${radiusSlug}"]`).val() || '0';
    ...
    function saveToJSON() {
        ...
        $.ajax({
            url: primitiveBorders.ajaxUrl,
            type: 'POST',
            data: {
                action: 'save_borders_primitive',
                nonce: primitiveBorders.nonce,
                borders_data: JSON.stringify(data)
            },
            beforeSend: function() {
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    showNotification('Borders saved successfully!', 'success');
                } else {
                    showNotification('Error: ' + response.data, 'error');
                }
            },
            error: function() {
                showNotification('Failed to save borders data', 'error');
            },
            complete: function() {
    ...
    function syncToThemeJSON() {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'sync_borders_to_theme_json',
                nonce: primitiveBorders.nonce,
                borders_data: JSON.stringify(data)
            },
            beforeSend: function() {
    ...
    function resetToDefaults() {
        if (!confirm('Are you sure you want to reset all border values to their defaults? This cannot be undone.')) {
            return;
        }
